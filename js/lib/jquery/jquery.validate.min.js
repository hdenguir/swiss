...
// methods added by void
// validate Email caracters
$.validator.addMethod("lettersonlyemail", function(value, element) {
	return this.optional(element) || /^[a-zA-Z0-9._-]+@[a-zA-Z0-9-]+\.{1,1}[a-zA-Z]{2,4}$/i.test(value);
}, ""); 

// validate password caracters / must use number(s) and letter(s)
$.validator.addMethod("passwordValide", function(value, element) {
	return ( value.match(/([a-z].*[A-Z])|([A-Z].*[a-z])/) != null && value.match(/([a-zA-Z])/) && value.match(/([0-9])/) && value.match(/([!,%,&,@,#,$,^,*,?,_,~,",',(,-,è,ç,à,),=,+,°])/)) ? true : false; 
}, ""); 
